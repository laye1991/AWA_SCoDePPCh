...
export default function SectorGuidesPage() {
  ...
  // Récupération des guides de chasse de la zone de l'agent secteur
  const { data: guides = [], isLoading } = useQuery<HuntingGuide[]>({
    queryKey: ["/api/hunting-guides/zone", user?.zone],
    enabled: !!user?.zone && user?.role === "sub-agent",
  });
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {user?.zone && (
            <p className="text-muted-foreground">Zone: {user.zone}</p>
          )}
        </div>
      </div>
